...
object Erros {
  ...
  private[this] object Erro {
    ...
    val variavelJaExiste: Regex = "(\\S+) is already defined as variable .+".r
    val alterarValorConstante: Regex = "reassignment to val \\| (\\S+).*".r
    val valorJaDeclarado: Regex = "(\\S+) is already defined as value.*".r
    val funcaoJaDefinida: Regex = "method (\\S+) is defined twice .*".r
    val funcaoRecursivaSemTipo: Regex = "recursive method (\\S+) needs result type.*".r
    val matrizNaoDeclarada: Regex = "value (?:get|update) is not (\\S+) member of.*".r
    val metodoNaoExiste: Regex = "value (\\S+) is not a member of (?:\\S*\\.)?(\\S+) .*".r
    val semParametros: Regex = "(.+) does not take parameters.*".r
    val divisaoTipoErrado: Regex = "overloaded method value.*".r
  }
